...
PipelineBuilder& PipelineBuilder::create_pipeline_layout()
{
    ...
    auto result = _device.handle().createPipelineLayout(&create_info, nullptr, &pipeline.pipeline_layout);
    return *this;
}
...
{
    shaders.push_back(std::make_unique<re::Shader>(shader_src, shader_stage, _device));
    return *this;
}
...
Pipeline PipelineBuilder::create_graphics_pipeline(const RenderPass& render_pass)
{
    ...
    vk::Result result;
    std::tie( result, pipeline.pipeline ) = _device.handle().createGraphicsPipeline(nullptr, create_info);
...
Pipeline PipelineBuilder::create_ray_tracing_pipeline(int ray_recursion_depth)
{
    ...
    auto result = _device.handle().createRayTracingPipelinesKHR(nullptr, nullptr, 1, &create_info,
                                                                nullptr, &pipeline.pipeline, _device.dispatch());
