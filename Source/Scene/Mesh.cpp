...
{
    m_index_buffer = std::make_unique<IndexBuffer>(m_geometry->indices(), command_buffers, m_device);
    m_vertex_buffer = std::make_unique<VertexBuffer>(m_geometry->vertices(), command_buffers, m_device);
    m_instance_buffer = std::make_unique<InstanceBuffer>(m_instances, command_buffers, m_device);
}
...
void Mesh::draw(vk::CommandBuffer& cmd_buffer)
{
    ...
    cmd_buffer.bindVertexBuffers(0, 1, &m_vertex_buffer->buffer().handle(), offsets.data());
