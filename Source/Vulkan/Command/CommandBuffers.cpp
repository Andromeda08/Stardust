...
CommandBuffer::CommandBuffer(const Device& device, uint32_t size)
: m_device(device)
{
...
vk::CommandBuffer CommandBuffer::begin(size_t index) const
{
    vk::CommandBufferBeginInfo begin_info;
    begin_info.setFlags(vk::CommandBufferUsageFlagBits::eSimultaneousUse);
    auto result = m_buffers[index].begin(&begin_info);
    return m_buffers[index];
}

void CommandBuffer::end(size_t index) const
{
    vkEndCommandBuffer(m_buffers[index]);
}

vk::CommandBuffer CommandBuffer::begin_single_time() const
{
    vk::CommandBufferAllocateInfo alloc_info;
    alloc_info.setLevel(vk::CommandBufferLevel::ePrimary);
...
void CommandBuffer::end_single_time(vk::CommandBuffer cmd_buffer) const
{
    cmd_buffer.end();
