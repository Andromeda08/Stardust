...
namespace sd
{
    ...
    {
    public:
        Scene2(const sdvk::CommandBuffers& command_buffers, const sdvk::Context& context, const sdvk::Swapchain& swapchain)
        : m_command_buffers(command_buffers), m_context(context), m_swapchain(swapchain)
        {
            ...
            m_rtao_kernel = RTAOKernel::Builder()
        ...
        void rasterize(uint32_t current_frame, vk::CommandBuffer const& command_buffer)
        {
            ...
            if (m_render_settings.ao_enabled)
            {
                m_rtao_kernel->run(m_camera->view(), command_buffer);
            }
            #pragma endregion
    ...
    private:
        ...
        void update_composite_descriptors()
        {
            ...
            vk::DescriptorImageInfo ao_buffer = { m_composite_render_target.sampler, m_rtao_kernel->get_result().view(), vk::ImageLayout::eShaderReadOnlyOptimal };
    ...
    private:
        ...
        std::unique_ptr<RTAOKernel> m_rtao_kernel;
