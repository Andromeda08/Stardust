...
namespace Nebula
{
                                                         ...
                                                         uint32_t count)
    {
        ...
        dbi.setBuffer(buffer);
        dbi.setRange(range);
        dbi.setOffset(offset);

        _buffer_infos.emplace_back(dbi);

        const auto* dbiptr = &_buffer_infos.back();
        assert(dbiptr->offset == offset);
